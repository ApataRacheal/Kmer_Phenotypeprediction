...
For this example, the sub-groups simply correspond to the geographic origin of the samples, but in practice one could consider alternative strategies based on actual population structure covariates (e.g., sequence types - STs) to define more or less challenging settings (e.g., maintaining the STs prevalences or considering distinct STs between the training and test sets).
...
We then extract the model into a dedicated object using the **extract_best_model()** function. This object contains the same information as for a lasso model, and an additionnal field providing the clusters definition.
...
This figure represents as a heatmap the correlation matrix built from the features selected by the lasso and the cluster-lasso (identifed by the orange and blue bars shown above the heatmap, respectively).
The corresponding (absolute) values of model coeffcients are represented by green bars. 
The  major clusters (involving more than $\texttt{clust.min}$ features, set here to $5$) of the cluster-lasso signatures are identifed by a dedicated color ranging from red to grey. 
